...
views.dash = () => {
  ...
  <div class="card next-list" style="margin-bottom:18px">
    ${lastCp ? nextItem(`עצרת בנושא ${(lastCp.cp.t??0)+1} מתוך ${byId[lastCp.sid].topics.length} · ${topicPct(lastCp.sid, lastCp.cp.t)}%`,
    ...
    <div class="grid g3" style="margin-bottom:8px">${list.map(s=>{
      ...
      return `<div class="card sum-card">
        ...
        ${cp ? `<button class="cp-hint" onclick="openReader('${s.sid}')" title="המשך מהמקום שעצרת">
          ${cp.pin?'📌':'↩︎'} המשך מנושא ${num((cp.t??0)+1+'/'+tot)} · ${num(topicPct(s.sid,cp.t))}%</button>` : ''}
...
views.reader = () => {
  ...
  <div class="rd-bar" role="region" aria-label="התקדמות וצ׳קפוינט">
    ...
    ${cp ? `<button class="btn sm" onclick="jumpCheckpoint()" title="קפוץ לצ׳קפוינט — נושא ${(cp.t??0)+1}">${cp.pin?'📌':'↩︎'} ${num(topicPct(s.sid,cp.t))}%</button>
...
/* המיקום מוצג ביחידות של נושאים ולא של פיקסלים: "נושא 4 מתוך 10" אומר משהו על
   החומר, בעוד ש"45% מהדף" משתנה לפי אורך הסעיפים ולא לפי כמה באמת עברת.
   אחוז הגלילה עדיין נשמר בצ'קפוינט (cp.p) — הוא מה שמחזיר אותך לנקודה המדויקת. */
function topicPct(sid, t){
  const s = byId[sid];
  if(!s || !s.topics.length || t == null) return 0;
  return Math.round((t + 1) * 100 / s.topics.length);
}
function updatePosLabel(){
  ...
  el.innerHTML = t == null
    ? 'מיקום: לפני הנושא הראשון'
    : `מיקום: נושא ${num((t+1)+'/'+s.topics.length)} · <b class="num">${topicPct(s.sid,t)}%</b>`;
}
